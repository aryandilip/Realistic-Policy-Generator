function [ModalControlResults] = ModalControl(A,B,e)
...
asuniq = unique(Srt);
[siuniq, swap] = size(asuniq) ;  %%Determining the number of unique eigenvalues
siuniq;

Ncount = histc(Srt, asuniq);  %%The number of repeatition for each unique eigenvalue.
uniq = sort(asuniq, 'descend'); %% Arranging the value of asunique in descending order
[SizeNcount, swap2] = size(Ncount);  %%Arranging the value of asNcount in descending order
if mod(SizeNcount,2) == 0
    halfofsizencount = SizeNcount/2;
    for i = 1:halfofsizencount
        justswap = Ncount(i,1);
        Ncount(i,1) = Ncount(SizeNcount-i+1,1)
        Ncount(SizeNcount-i+1,1) = justswap
    end
else
    halfofsizencount = (SizeNcount-1)/2;
    for i = 1:halfofsizencount
        justswap = Ncount(i,1);
        Ncount(i,1) = Ncount(SizeNcount-i+1,1)
        Ncount(SizeNcount-i+1,1) = justswap
...
for i = 1:x
    for j = 1:x
        eqn(1,j + bnull) = leftside(i,j) == rightside(i,j);
    end
    bnull = bnull+x;
end
...
elseif IfAny > 0
    ...
    ModalControlResults = Combined_equation;

...
NonZero = all(p(value+1:value+Ncount(output,1),InputPreferred));
NonZeroMinusOne = all(p(value+1:value+Ncount(output,1)-1,InputPreferred));
if NonZero == 1
                
elseif (NonZero == 0) && (NonZeroMinusOne == 1)
    Ncount(output,1) = Ncount(output,1)-1;
else
    fprintf('ERROR: The Jordan Matrix cannot be controlled!')
    return;
end
